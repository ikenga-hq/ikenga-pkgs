...
3. **Build the board data model** — get the base model from the script (`groundwork_state.py board-data --plan <plan>`), which derives `plan` / `gates` / `wps` / `designs` / `subplans` / `research` from the anchor + `01`, plus the design lifecycle: `design_lifecycle` (one entry per `D-NN` with `design_state`, `build_state`, implementing `wps`, `prs`, `findings`, `warnings`), `design_unlinked` (variant files with no `D-NN`), `coverage` (per phase), and per-WP `implements`. Enrich it with the per-WP `brief` text extracted from `09-orchestration.md` and the `decisions`/`waves` views the board needs. Pass `design_lifecycle` and `coverage` through unchanged — the rail renders them (§"The board UI"). The shape:
   ```json
   {
